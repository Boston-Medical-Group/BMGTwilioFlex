...
exports.handler = FunctionTokenValidator(async function (
    ...
    try {
        ...
        const conversationContext = context.getTwilioClient().conversations.v1.conversations(event.conversationSid)
        await conversationContext.fetch().then(async (data) => {
            let participants = await conversationContext.participants.list()
            sendResponse = {
                ...data.toJSON(),
                participants
            }
